...
function Line_Intersection_finite(p1, pdir, v1, vdir) { //do not give normalised value of direction vectors because then bounds cannot be calculated properly
    const p2 = vec_add(p1, pdir);
    const v2 = vec_add(v1, vdir);
    ...
    const intersection = new Vec2d(Px, Py);

    // Helper to check if point lies within segment bounds
    ...
    if (pointOnSegment(intersection, p1, p2) && pointOnSegment(intersection, v1, v2)) {
        return intersection;
    }
...
function pointChecker_shape( point,  frame){  //winding order of the shape should be anti clockwise and polygon should be convex

    let shape_size = frame.length;
    
    for(let i=0; i < shape_size ;i++){
    
        let p = frame[i];
        let q = frame[(i+1) % shape_size];
    
        let refv = vec_sub(q,p);
...
//GPT :)
function FillTriangle(vertexA, vertexB, vertexC, fillCol = 'black', lineCol = 'red', lw = 5,) {
